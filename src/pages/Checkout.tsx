...
export default function Checkout() {
  ...
  useEffect(() => {
    if (!id) return;
    supabase.from("game_listings").select("*, seller:profiles(full_name, username, is_verified)").eq("id", id).maybeSingle()
      .then(({ data }) => { setListing(data as GameListing); setLoading(false); });
  }, [id]);
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const { error: insErr } = await supabase.from("orders").insert({
      ...
      price: listing.price, payment_method: method, payment_number: paymentNumber || null,
    });
    setSubmitting(false);
    ...
    <div className="mx-auto max-w-2xl px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="card p-5 mb-4">
        <div className="flex gap-3">
          ...
          <p className="font-display text-xl font-extrabold text-white">{formatBDT(listing.price)}</p>
        </div>
      </div>
      <form onSubmit={handleSubmit} className="card p-6 space-y-5">
        ...
        <div className="rounded-xl bg-ink-800 p-4 space-y-2 text-sm">
          <div className="flex justify-between text-ink-400"><span>Price</span><span className="text-white">{formatBDT(listing.price)}</span></div>
          ...
          <div className="flex justify-between font-bold text-base border-t border-ink-700 pt-2"><span className="text-white">You Pay</span><span className="text-primary-400">{formatBDT(listing.price)}</span></div>
        </div>
        ...
        <button type="submit" disabled={submitting} className="btn-primary w-full">{submitting ? <Loader2 size={18} className="animate-spin" /> : `Pay ${formatBDT(listing.price)}`}</button>
      </form>
    </div>
  );
}
